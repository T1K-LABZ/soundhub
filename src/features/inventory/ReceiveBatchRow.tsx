...
import type { StockBatchItem } from "./inventory.types";
...
type Props = {
  ...
  onChange: (updated: StockBatchItem) => void;
  onRemove: (itemId: string) => void;
};

export function ReceiveBatchRow({
  item,
  index,
  products,
  canRemove,
  ...
  function set<K extends keyof StockBatchItem>(k: K, v: StockBatchItem[K]) {
    onChange({ ...item, [k]: v });
  }

  // When a product is selected, pre-fill prices from the existing product record
  // as a starting suggestion — the user can still edit them.
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
        ...
        <TextField
          ...
          label="Status"
          value={item.status}
          onChange={(e) => set("status", e.target.value as "PENDING" | "ACTIVE")}
          size="small"
      ...
      {/* Margin preview — shows instantly as user types */}
